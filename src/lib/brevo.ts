...
<body style="margin:0;padding:0;background:#ece9e2;font-family:sans-serif;">
      ...
      <table width="520" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:18px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08);">
        ...
        <tr><td style="padding:36px 40px;">
          <p style="margin:0 0 16px;font-size:15px;color:#333;line-height:1.7;">
            Вы запросили восстановление доступа к платформе INCF.
          </p>
          <p style="margin:0 0 28px;font-size:14px;color:#666;line-height:1.7;">
            Нажмите на кнопку ниже, чтобы задать новый пароль. Ссылка действует <strong>1 час</strong>.
          </p>
        </td></tr>
  ...
  const body = {
    ...
    to: [{ email }],
    subject: 'Восстановление доступа — Введение в нейрокоучинг',
    htmlContent,
  }
...
<body style="margin:0;padding:0;background:#ece9e2;font-family:sans-serif;">
      ...
      <table width="520" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:18px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08);">
        ...
        <tr><td style="padding:36px 40px;">
          ...
          <p style="margin:0 0 16px;font-size:14px;color:#666;line-height:1.7;">
            Пароль для вашей учётной записи на платформе INCF был успешно обновлён.
          </p>
          <p style="margin:0 0 28px;font-size:14px;color:#666;line-height:1.7;">
  ...
  const body = {
    sender: { name: 'INCF Нейрокоучинг', email: 'noreply@incf.eu' },
    to: [{ email }],
    subject: 'Ваш пароль успешно изменён — INCF',
    htmlContent,
  }
...
export async function sendAlreadyEnrolledEmail(email: string): Promise<void> {
  const apiKey = process.env.BREVO_API_KEY
  if (!apiKey) throw new Error('Missing BREVO_API_KEY')
...
<body style="margin:0;padding:0;background:#ece9e2;font-family:sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#ece9e2;padding:40px 0;">
    <tr><td align="center">
      <table width="520" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:18px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08);">
        ...
        <tr><td style="padding:36px 40px;">
          <p style="margin:0 0 16px;font-size:15px;color:#333;line-height:1.7;font-weight:600;">
            У вас уже есть доступ к платформе
          </p>
          <p style="margin:0 0 16px;font-size:14px;color:#666;line-height:1.7;">
            Ваш аккаунт на платформе INCF уже активен. Войдите с помощью email и пароля, которые вы использовали ранее.
          </p>
          <p style="margin:0 0 28px;font-size:14px;color:#666;line-height:1.7;">
            Если не помните пароль — воспользуйтесь функцией восстановления на странице входа.
          </p>
        </td></tr>
        ...
        <tr><td align="center" style="padding:0 40px 36px;">
          <a href="${siteUrl}/login" style="font-size:13px;color:#4a7c8e;text-decoration:none;">
  ...
  const body = {
    sender: { name: 'INCF Нейрокоучинг', email: 'noreply@incf.eu' },
    to: [{ email }],
    subject: 'У вас уже есть доступ к платформе INCF',
    htmlContent,
  }
